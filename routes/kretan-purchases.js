...
router.get('/', (req, res) => {
  ...
  const purchases = list.slice(offset, offset+parseInt(limit)).map(p => {
    const c = db.find('contacts', p.customer_id);
...
router.post('/', (req, res) => {
  ...
  const cust = db.find('contacts', parseInt(customer_id));
  if (cust) db.update('contacts', cust.id, { loyalty_points:(cust.loyalty_points||0)+pts, total_spent:(cust.total_spent||0)+total_amount, visit_count:(cust.visit_count||0)+1, updated_at:new Date().toISOString() });
...
router.post('/redeem', (req, res) => {
  ...
  db.update('contacts', cust.id, { loyalty_points: cust.loyalty_points - points });
